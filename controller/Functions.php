    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkEmail) > 0) {
        ...
        if ($data['password'] !== $data['re_password']) {
          ...
          $en_user = str_replace(".", "", $en_user);
          $to       = $data['email'];
          $subject  = "Account Verification - SI Penjualan Bahan Bangunan";
                        ...
                        <tr>
                            ...
                            <table role='presentation' border='0' cellpadding='0' cellspacing='0' style='border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; min-width: 100%; width: 100%;' width='100%'>
                                ...
                                <td style='font-family: sans-serif; font-size: 14px; vertical-align: top;' valign='top'>
                                    ...
                                    <p style='font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;'>Terima kasih telah mendaftar di SI Penjualan Bahan Bangunan.</p>
  ...
  function forgot_password($conn, $data, $action, $baseURL)
  {
    if ($action == "update") {
      ...
      if (mysqli_num_rows($checkEmail) === 0) {
        ...
        $en_user = str_replace(".", "", $en_user);
        $to       = $data['email'];
        $subject  = "Reset password - SI Penjualan Bahan Bangunan";
        $message  = "<!doctype html>
        <html>
          <head>
              <meta name='viewport' content='width=device-width'>
...
if (isset($_SESSION["project_si_penjualan_bahan_bangunan"]["users"])) {
  ...
  function pelanggan($conn, $data, $action)
  {
    $result = 0;

    if ($action == "update") {
      // 1. Update tabel users
      $sql_users = "UPDATE users SET name='$data[name]', email='$data[email]', id_active='$data[id_active]' WHERE id_user='$data[id_user]'";
      mysqli_query($conn, $sql_users);
      $result += mysqli_affected_rows($conn);

      // 2. Insert atau Update tabel_pelanggan
      if (empty($data['id_pelanggan'])) {
        $sql_pelanggan = "INSERT INTO tabel_pelanggan (id_user, alamat, nomor_telepon) VALUES ('$data[id_user]', '$data[alamat]', '$data[nomor_telepon]')";
      } else {
        $sql_pelanggan = "UPDATE tabel_pelanggan SET alamat='$data[alamat]', nomor_telepon='$data[nomor_telepon]' WHERE id_pelanggan='$data[id_pelanggan]'";
      }
    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkMenu) > 0) {
        ...
        if (!is_dir($pathFolder)) {
          ...
          fwrite($file, '<?php if (!isset($_SESSION["project_si_penjualan_bahan_bangunan"]["users"])) {
  ...
  function sub_menu($conn, $data, $action, $baseURL)
  {
    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkSubMenu) > 0) {
        ...
        $_SESSION["project_si_penjualan_bahan_bangunan"]["name_page"] = "' . $data['title'] . '";
        ...
        $_SESSION["project_si_penjualan_bahan_bangunan"]["name_page"] = "Tambah ' . $data['title'] . '";
        require_once("../../templates/views_top.php"); ?>
        ...
        $_SESSION["project_si_penjualan_bahan_bangunan"]["name_page"] = "Ubah ' . $data['title'] . '";
        require_once("../../templates/views_top.php"); ?>
        ...
        <div class="nxl-content" style="height: 100vh;">
          ...
          <div class="page-header">
            <div class="page-header-left d-flex align-items-center">
              ...
              <ul class="breadcrumb">
                <li class="breadcrumb-item">' . $data['title'] . '</li>
                <li class="breadcrumb-item"><?= $_SESSION["project_si_penjualan_bahan_bangunan"]["name_page"].' . $petik . ' ' . $petik . '.$view_data[""]  ?></li>
              </ul>
            </div>
          </div>
          <!-- [ page-header ] end -->
  ...
  function barang($conn, $data, $action)
  {
    ...
    if ($action == "insert") {
      ...
      if (mysqli_num_rows($checkKode) > 0) {
        $message = "Maaf, Kode Barang yang Anda masukkan sudah terdaftar.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      }
  ...
  function inventori($conn, $data, $action)
  {
    if ($action == "insert") {
      // Validasi backend: Memastikan barang belum ada di inventori
      $checkKatalog = "SELECT * FROM tabel_inventori WHERE kode_katalog = '$data[kode_katalog]'";
      $checkKatalog = mysqli_query($conn, $checkKatalog);
      if (mysqli_num_rows($checkKatalog) > 0) {
        $message = "Maaf, data stok untuk barang ini sudah ada.";
        $message_type = "danger";
        alert($message, $message_type);
        return false;
      } else {
        $sql = "INSERT INTO tabel_inventori (kode_katalog, stok_tersedia) 
              VALUES ('$data[kode_katalog]', '$data[stok_tersedia]')";
      }
    }

    if ($action == "update") {
      // Kita menggunakan id_inventori sebagai acuan untuk update agar lebih aman
      $sql = "UPDATE tabel_inventori 
            SET stok_tersedia='$data[stok_tersedia]' 
            WHERE id_inventori='$data[id_inventori]'";
    }

    if ($action == "delete") {
      $sql = "DELETE FROM tabel_inventori WHERE id_inventori='$data[id_inventori]'";
    }
  ...
  function pesanan($conn, $data, $action)
  {
    ...
    if ($action == "update") {
      ...
      // Cek status pesanan saat ini (sebelum diupdate)
      $cek_status = mysqli_query($conn, "SELECT status_pesanan FROM tabel_pesanan WHERE id_pesanan = '$id_pesanan'");
      $row_status = mysqli_fetch_assoc($cek_status);
      $status_lama = $row_status['status_pesanan'];

      // Logika Pengurangan / Pengembalian Stok
      if ($status_lama == 'Menunggu Konfirmasi' && ($status_baru == 'Diproses' || $status_baru == 'Selesai')) {
        // Kurangi stok karena pesanan mulai diproses
        $transaksi = mysqli_query($conn, "SELECT kode_katalog, jumlah_barang FROM tabel_transaksi WHERE id_pesanan = '$id_pesanan'");
        while ($item = mysqli_fetch_assoc($transaksi)) {
          mysqli_query($conn, "UPDATE tabel_inventori SET stok_tersedia = stok_tersedia - " . $item['jumlah_barang'] . " WHERE kode_katalog = '" . $item['kode_katalog'] . "'");
        }
      } else if (($status_lama == 'Diproses' || $status_lama == 'Selesai') && $status_baru == 'Batal') {
      ...
      // Update status di tabel_pesanan
      $sql = "UPDATE tabel_pesanan SET status_pesanan='$status_baru' WHERE id_pesanan='$id_pesanan'";
      mysqli_query($conn, $sql);
      $result += mysqli_affected_rows($conn);
    }

    if ($action == "delete") {
      // Karena di database kamu sudah menggunakan ON DELETE CASCADE pada tabel_transaksi,
      // menghapus id_pesanan di sini akan otomatis menghapus rincian barangnya juga.
      $sql = "DELETE FROM tabel_pesanan WHERE id_pesanan='$data[id_pesanan]'";
      mysqli_query($conn, $sql);
      $result += mysqli_affected_rows($conn);
    }
  ...
  function penjualan($conn, $data, $action, $id_user)
  {
    if ($action == 'insert') {
      // Sanitasi data opsional
      $nama_pelanggan = valid($conn, $data['nama_pelanggan']);
      $catatan = valid($conn, $data['catatan']);

      if (empty($nama_pelanggan)) {
        $nama_pelanggan = "Pelanggan Umum (Walk-in)";
      }

      // Insert ke tabel_pesanan (tambahkan tanggal_pesan)
      $query_pesanan = "INSERT INTO tabel_pesanan (id_user, status_pesanan, tanggal_pesan) 
                      VALUES ('$id_user', 'Selesai', CURRENT_TIMESTAMP)";

      if (mysqli_query($conn, $query_pesanan)) {
        $id_pesanan = mysqli_insert_id($conn);

        $kode_pilih = $data['kode_katalog_pilih'];
        $qty_array = $data['qty'];

        // Looping insert rincian barang
        foreach ($kode_pilih as $kode) {
          $kode_bersih = valid($conn, $kode);
          $qty = (int)$qty_array[$kode_bersih];

          if ($qty > 0) {
            // Ambil harga dari database
            $cek_katalog = mysqli_query($conn, "SELECT harga_barang FROM tabel_katalog WHERE kode_katalog = '$kode_bersih'");
            $row_katalog = mysqli_fetch_assoc($cek_katalog);
            $harga_satuan = $row_katalog['harga_barang'];

            $total_harga = $harga_satuan * $qty;

            // Insert ke tabel_transaksi (tambahkan tanggal_transaksi)
        ...
        return $id_pesanan; // Sukses, kembalikan ID Pesanan
      }
    }
    return 0; // Gagal
  }
